...
function Questions() {
    ...
    useEffect(function () {
        const getChoiceQuestions = async function () {
            const res = await fetch("http://snf-60107.vm.okeanos-global.grnet.gr:3001/question/cPlusPlus/Medium/Choice", { credentials: "include" });
            //wait for json data
        ...
        const getShortQuestions = async function () {
            const res = await fetch("http://snf-60107.vm.okeanos-global.grnet.gr:3001/question/cPlusPlus/Medium/Short", { credentials: "include" });
            //wait for json data
            const data = await res.json();
             //set variable to value of data
